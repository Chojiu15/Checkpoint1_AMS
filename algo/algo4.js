...
const people = [
  ...
  function findAdults(people) {
    // Your code here !
    const adultsArray = people.filter(people => people.age > 18)
    const adultFemales = adultsArray.filter(people => people.sex === "female")
    const adultMales = adultsArray.filter(people => people.sex === "male")
    const adultFemalesAndMales = [adultFemales, adultMales]
    
    return adultFemalesAndMales
  }
